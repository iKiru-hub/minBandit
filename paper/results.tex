...
This was accomplished by evaluating the performances in a stationary setting with $K=10$ and increasing levels of entropy in the reward distribution, again measured in nats.
For more details about the calculation see the appendix \ref{sec:appendix_entropy}.
In the top plot of figure \ref{fig:entropy_distr}, it is plotted the average reward obtained by each model against the reward distribution entropy.
The results report how all models, with a mild expection of Epsilon-Greedy, are capable of robust performance even in the presence of clear uncertainty.
Nonetheless, our model show to suffer more than the others from the presence of high entropy. This fact is highlighed by the dashed lines representing the average regret, which overall keep getting wider despite a diminishing of the upper bound.
Another perspective to this analysis is given by the bottom plot, which shows the average entropy of the selections for the first and second trial of the simulation, showed respectively with solid and dashed lines.
...
\begin{figure}[H]
    ...
    \caption{\textsc{Entropy analysis for the model in a stationary setting} - Top plot: \textit{a solid line represent the average reward obtained by a model for for increasing levels of entropy in the reward distribution; a dashed line instead report the regret with respect to the upper bound
    (black solid line) } Bottom plot: \textit{average entropy of the selections for the first and second trial of the simulation, each with 2000 rounds each (as calculated in \ref{sec:entropy}).}}
