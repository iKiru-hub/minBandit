...
The model has been tested and compared with the other algorithms: Thompson Sampling (TS), $\epsilon$-Greedy, and UCB. The benchmark were the four different variants of the MAB problem listed above \ref{sec:envs}, with a variable number of arms ranging from 5 to 1000.
...
& \textit{Model} & $\mathit{0.08(13)}$ & $\mathit{0.07(11)}$ & $\mathit{0.07(14)}$ & $\mathit{0.07(8)}$ & $\mathit{0.09(9)}$ & $\mathbf{0.07(8)}$ \\
\midrule
...
& \textit{Model} & $\mathit{0.13(10)}$ & $\mathit{0.15(16)}$ & $\mathbf{0.05(6)}$ & $\mathit{0.21(5)}$ & $\mathbf{0.26(7)}$ & $\mathbf{0.12(7)}$ \\
\midrule
...
& \textit{Model} & $\mathbf{0.00(3)}$ & $\mathbf{0.02(4)}$ & $\mathbf{0.05(3)}$ & $\mathbf{0.06(4)}$ & $\mathit{0.08(1)}$ & $\mathbf{0.05(4)}$ \\
\midrule
...
& \textit{Model} & $\mathbf{0.00(9)}$ & $\mathbf{0.23(17)}$ & $\mathbf{0.14(7)}$ & $\mathbf{0.08(8)}$ & $\mathbf{0.06(4)}$ & $\mathbf{0.09(7)}$ \\\\
\bottomrule
\end{tabular}
...
Indeed, the dynamics of our model make it particularly suited for the task of non-stationary MAB, as it is able to quickly adapt to new reward distributions and firmly maintain a greedy policy.
...
% robustness
\noindent Then, we sought to investigate the robustness of the model by targeting the capacity to endure increasing levels of entropy in the reward distribution, defined as $-\sum^{N}_{i} p_{i} \log(p_{i})$ and calculated in nats.
In particular, this analsysis tracked the performance, cumulative reward and regret, as well as the selection entropy, defined as the entropy of the distribution of selected options within a sliding window of $20$ rounds.
