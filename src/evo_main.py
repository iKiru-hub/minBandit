...
from utils import setup_logger, make_probability_set, CONFIG_PATH
from envs import make_new_env
...
if __name__ == "__main__" :
    ...
    with open(CONFIG_PATH + "/config_evo.yaml", 'r') as stream:
        try:
    ...
    with open(CONFIG_PATH + "/config_model.yaml", 'r') as stream:
        try:
    ...
    evolution.USE_TQDM = False
    evolution.FORCE_POOL = False
    ...
    # Create the toolbox
    toolbox = evolution.make_toolbox(PARAMETERS=PARAMETERS.copy(),
                                     game=env,
                                     model=model,
                                     strategy=strategy,
    ...
    if not args.noplot:
        visualizer = evolution.Visualizer(settings=settings,
                                          online=False,
                                          target=None,
                                          k_average=10,
    ...
    best_ind = evolution.main(toolbox=toolbox, settings=settings,
